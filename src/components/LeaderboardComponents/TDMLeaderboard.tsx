...
const TDMLeaderboard = () => {
  ...
  const [sortConfig, setSortConfig] = useState<SortConfig<ComputedPlayerData>>({
    key: "rank",
    direction: "ascending",
  });
  ...
  const updateData = (data: RawTDMPlayerData[]) => {
    ...
    for (let i = 0; i < data.length; i++) {
      const playerData: ComputedPlayerData = {
        ...
        kd_ratio:
          data[i].tdm_total_deaths > 0
            ? data[i].tdm_total_kills / data[i].tdm_total_deaths
            : 0,
        rank: 0,
      };
